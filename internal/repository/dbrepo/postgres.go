...
func (m *postgresDBRepo) GetUserByID(id int) (models.User, error) {
	...
	query := `select 
       			r.id, r.full_name, r.email, r.phone, r.ktp_images, r.password, r.address_id, r.role_id, r.created_at, r.updated_at,
       			ru.id, ru.name
             	from users r
             	left join roles ru on (r.role_id = ru.id)
...
func (m *postgresDBRepo) RegisterUser(u models.User) (int, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	var newID int

	query := `insert into users (full_name, email, phone, ktp_images, created_at, updated_at)
...
func (m *postgresDBRepo) UpdateUser(u models.User) error {
	...
	query := `update users set full_name = $1, email =$2, phone = $3, ktp_images = $4, updated_at = $5`
...
// DeleteReservations deletes one reservations by id
func (m *postgresDBRepo) DeleteReservation(id int) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	query := "delete reservations where id = $1"
...
// DeleteBlockForRoom delete a room restriction
